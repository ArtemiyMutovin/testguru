class BadgeIssuance
  ...
  def category(badge)
    ...
    category_check(user_tests, last_badge_date, category_tests_ids)
  end
  ...
  def level(badge)
    ...
    level_check(user_tests, last_badge_date, level_tests_ids)
  end

  private

  def category_check(user_tests, last_badge_date, category_tests_ids)
  ...
  def level_check(user_tests, last_badge_date, level_tests_ids)
    user_tests_ids = if last_badge_date.present?
                       user_tests.where('test_passages.created_at > ?', last_badge_date).select(:id)
                     else
                       user_tests.select(:id)
